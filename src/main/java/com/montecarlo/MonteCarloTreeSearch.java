...
public abstract class MonteCarloTreeSearch<T extends Transition, N extends Node<T>> {
  ...
  /**
   * This is where we are. Each {@link Node} keeps a reference to its parent {@link Node} and to each child {@link
   * Node}.
   */
  ...
  /**
   * Get the best {@link Transition} for the current player. Playing a {@link Transition} MUST be done by calling {@link
   * #makeTransitionAndChangeRoot(Transition)} unless next call to this method WILL rely on a wrong origin.
   *
  ...
  /**
   * Truncate the tree, keeping only the current root {@link Node} and its sub-tree. Sub-trees that does not contains
   * the current root {@link Node} are removes as well.
   */
  ...
  /**
   * Update the context and change the root of the tree to this context so that it reflects the realization of the given
   * {@link Transition}. This method is the same as {@link #makeTransition(Transition)} but it also change the root of
   * the tree to the {@link Node} reached by the given {@link Transition}. MUST only be called with a {@link Transition}
   ...
   * realization of the given {@link Transition}. This method is the same as {@link #unmakeTransition(Transition)} but
   * it also change the root of the tree to the origin {@link Node} of the given {@link Transition} in the tree. MUST
   * only be called with the last {@link Transition} passed to {@link #makeTransition(Transition)}.
   *
   * @param transition
  ...
  /**
   * Select a non terminal leaf {@link Node} to expand expressed by a {@link Path} from the current root {@link Node} to
   * the leaf {@link Node}. The selection is done by calling {@link #selectNonTerminalChildOf(Node)} from child to child
   * until we reach a leaf {@link Node}.
   *
   ...
   * @return the {@link Path} to the leaf {@link Node} to expand or null if there's nothing else to expand...
   */
  ...
  /**
   * Expand the leaf {@link Node} by creating <strong>every</strong> child {@link Node}.<br/> The leaf {@link Node} to
   * expand MUST NOT be a terminal {@link Node}.<br/> After expansion, the leaf {@link Node} has all of its children
   * created.<br/>
   *
   * @param path a {@link Path} to a non terminal leaf {@link Node}.
   * @return the {@link Path} to run the random simulation from. The expanded {@link Node} MIGHT be a terminal {@link
   * Node}
   */
  private Path<T, N> expansion(final Path<T, N> path) {
    ...
    if (!possibleTransitions.isEmpty()) {
      ...
      throw new IllegalStateException("Trying to expand a " + Node.class.getName() + " with no possible transitions. "
                                          ...
                                          + "Check the contract for " + MonteCarloTreeSearch.class.getName()
   ...
   * @return the next (non terminal) {@link Node} in the selection step or null if there's no more child node to explore
   * re or null if there's only terminal child nodes
   * @see Node#isTerminal()
   */
   ...
   *
   * @param possibleTransitions The possible transitions from the current root {@link Node}
   * @return The desired {@link Transition} or null if possibleTransitions is null or empty
   */
   ...
   *
   * @param possibleTransitions Possible {@link Transition} of the selected {@link Node} At least one possible {@link
   *                            Transition}
   * @return The desired {@link Transition} to get the expanded {@link Node} from
   */
  public abstract T expansionTransition(Set<T> possibleTransitions);

  /**
   * Update the context so it takes into account the realization of the given {@link Transition}. MUST only be called
   * with a {@link Transition} returned by {@link #getBestTransition()}.
   *
   * @param transition A {@link Transition} returned by {@link #getBestTransition()} or null
   * @see #doTransition(Transition)
   */
  protected abstract void makeTransition(T transition);

  /**
   * Update the context so it takes into account the rollback of the given {@link Transition}. MUST only be called with
   * the last {@link Transition} passed to {@link #makeTransition(Transition)}.
   *
   * @param transition A {@link Transition} returned by {@link #getBestTransition()} or null
  ...
  /**
   * Get possible transitions from the current position. Returned transitions MIGHT involves any of the players, taking
   * into account actions such as pass, skip, fold, etc...
   *
   * @return possible transitions for the current position or an empty {@link Set} if there's no possible transition.
   */
  ...
  /**
   * Change current turn to the next player. This method must not be used in conjunction with the makeMove() method. Use
   * it to implement a <strong>pass</strong> functionality.
   *
   * @see #makeTransition(Transition)
   */
  ...
  /**
   * Change current turn to the previous player. This method must not be used in conjunction with the unmakeMove()
   * method. Use it to implement an <strong>undo</strong> functionality.
   *
   * @see #unmakeTransition(Transition)
   */
