...
public abstract class Node<T extends Transition> {
  ...
  /**
   * A leaf {@link Node} is a node with no child. There's two case where a {@link Node} can be leaf :
   * <ol>
  ...
  /**
   * A {@link Node} is terminal when there is no child to explore. The sub-Tree of this {@link Node} has been fully
   * explored or the {@link Node} correspond to a configuration where {@link MonteCarloTreeSearch#isOver()} return
   * true.
   *
   * @return
   */
  ...
  /**
   * Propagate the result of a simulation to this {@link Node}. After a call to this method, {@link #simulations()} is
   * incremented as well as {@link #wins(int)} for the given winner.
   *
  ...
  /**
   * Get the value of the {@link Node} for the given player. The {@link Node} with the greater value will be picked as
   * the best choice for this player.
   *
   * @param player
   * @return
   */
   ...
   *
   * @return The return {@link Map} MUST NOT be null If the {@link Node} is the leaf {@link Node}, then an empty {@link
   * Map} is returned
   */
   ...
   *
   * @return The return {@link Collection} MUST NOT be null If the {@link Node} is the leaf {@link Node}, then an empty
